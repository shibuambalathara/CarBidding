...
export const Event = list({
  ...
  // create & update operaration updated by Shibu....
  access: {
    operation: {
      query: () => true,
  ...
  hooks: {
    validateInput: async ({
      ...
      if (new Date(resolvedData.endDate) < new Date() && item.eventCategory!='stock') {
    ...
    resolveInput: async ({
      ...
      console.log("resolved data on event",resolvedData)
    ...
    afterOperation: async ({
      ...
      if (
        ...
        const t = originalItem.pauseDate
          ...
          : new Date().getTime();
        const pause=new Date(originalItem.pauseDate as Date).toJSON()
     
          // update the vehicle end time'
        await context.prisma.$executeRawUnsafe(`UPDATE "Vehicle" set "bidTimeExpire" = "bidTimeExpire" + ${t} * INTERVAL '1 MILLISECOND', "bidStartTime" = "bidStartTime" + ${t} * INTERVAL '1 MILLISECOND' WHERE "event" = '${originalItem.id}' AND "bidTimeExpire" > '${pause}'::timestamp `);
        await context.prisma.$executeRawUnsafe(`UPDATE "Event" set "endDate" = "endDate" + ${t} * INTERVAL '1 MILLISECOND' WHERE id =  '${originalItem.id}'`)
      }
    },
  },
  ...
  fields: {
    ...
    eventCategory: select({
      options: [
        ...
        { value: "stock", label: "stock Auction" },
