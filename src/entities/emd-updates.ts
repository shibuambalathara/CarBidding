...
export const EmdUpdate = list({
  ...
  hooks: {
    ...
    afterOperation: async ({ resolvedData, context, operation,item }) => {
      if (operation !== "create") {
        return;
      }
     const { id: emdUpdateId } = item; 
      ...
      await context.prisma.user.update({
       ...
       const {vehicleBuyingLimitIncrement,user,payment} =resolvedData
     
       const voucherCode=voucherCodes.generate({ prefix: "Auto-", postfix: "-2023",count:vehicleBuyingLimitIncrement})
    
       
   
       await  context.prisma.coupen.createMany({
        data: Array.from({ length: vehicleBuyingLimitIncrement }, (_, index) => ({
          coupenNumber:voucherCode[index],
          userDetailId:user.connect.id,
          paymentDetailId:payment.connect.id
