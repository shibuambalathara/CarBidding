...
import { fieldOptions, isNotAdmin, isSuperAdmin,isStaff } from "../application/access";
import excelFileToJson from "../services/excelFileToJson";
import { Vehicle } from "@prisma/client";
import { ImageFinding, containsNumbers, lastFourMonts } from "../lib/excel-upload-related functions";
...
// create & update operation updated by shibu
        ...
        try {
          const [data, eventData] = await Promise.all([
            ...
            context.query.Event.findOne({
              ...
              query: `endDate startDate eventCategory vehicleLiveTimeIn gapInBetweenVehicles seller{name} location{name}`,
            }),
          ]);


          // ---------------------------------find already uploaded images of corresponding vehicles


const formattedFourMonthsAgo =await lastFourMonts()

          
       const vehicleDetails = await context.prisma.Vehicle.findMany({
        
        
          where: { event: {seller: {name: {equals: eventData?.seller?.name } },
              AND: [
                { location: { name: { equals:eventData?.location?.name } }}
              ]
            },
            AND: [
              { createdAt: { gte: formattedFourMonthsAgo } }
            ]
          }
        
        
});
const eventDataVehicls = vehicleDetails.map((vehicle) => ({

   loanAgreementNumber:vehicle?.loanAgreementNo,
    frontImage: vehicle?.frontImage,
    registerNumber:vehicle?.registrationNumber

}));

const findImage =  (loan_agreement_no:string,regNo:string)=> {
 const result= ImageFinding   (loan_agreement_no,regNo,eventDataVehicls)
 return result
}
            ...
            const vehicles: Vehicle[] = data.map(
              (vehicle:any, i: number) => {
                ...
                return {
                  ...
                  engineNo: vehicleItem.engine_no?.toString(),
                  chassisNo: vehicleItem.chassis_no?.toString(),
                  frontImage:
                   findImage( vehicleItem.loan_agreement_no?.toString()?.trim(),vehicleItem.registration_number?.toString()?.trim()) ,
                  ...
                  bidTimeExpire:
                   ...
                   !lastVehicle ?(
                    eventData?.eventCategory === "open"
                      ...
                      :
                        // added by have existing vehicle shibu.................
                        eventData?.eventCategory === "open"
                      ? new Date(
                            new Date(lastVehicle?.bidTimeExpire).getTime() +
                              // eventData.vehicleLiveTimeIn * 60000 +
                              (eventData.vehicleLiveTimeIn * 60000 +
                                eventData.gapInBetweenVehicles * 1000) *
                                (i+1)
                      ...
                      : new Date(
                            new Date(lastVehicle?.bidTimeExpire).getTime() +
                              eventData?.gapInBetweenVehicles * 60000 * (i+1)
                          ),
                  //         // -----------------
                  bidStartTime:
                 
                  !lastVehicle ?       (eventData?.eventCategory === "open"
                      ? new Date(
                          new Date(eventData.startDate).getTime() +
                            (eventData?.vehicleLiveTimeIn * 60000 +
                              eventData?.gapInBetweenVehicles * 1000) *
                      ...
                      //---------------- have existing vehicle by shibu
                      :(eventData?.eventCategory === "open"
                      ? new Date(
                          new Date(lastVehicle?.bidTimeExpire).getTime() +
                          (eventData?.gapInBetweenVehicles * 1000) +
